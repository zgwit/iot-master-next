...
func (program *Program) GetLog(log_file_path string, row int) (contents []string, err error) {
	...
	return utils.ReadFileTail(log_file_path, row)
}
...
func (program *Program) loging(log_file_path string) {
	...
	for !program.exited_log {
		...
		if names, err := utils.GetDirsNames("./log/"); err != nil {
		...
		if err := utils.CopyFile(log_file_path, fmt.Sprintf("%s.%d", log_file_path, number)); err != nil {
